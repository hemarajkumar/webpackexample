<template>
  <div class="owl-carousel owl-theme js-homePageCarousel">
    <template v-for="lists in carouselobj">
      <template v-for="item in lists">
        <div class="item">
          <picture class="owl-item__picture">
             <template v-for="picture in item.list">
            <source v-bind:key="picture.imagetype" v-if="picture.imagetype == 'desktop'" 
                v-bind:srcset="picture.path"
                media="(min-width: 900px)" />
            <source v-bind:key="picture.imagetype" v-if="picture.imagetype == 'tablet'"
              srcset="dist/images/carousel/image1-tablet.png"
              media="(min-width: 641px) and (max-width: 899px)"
            />
            <source v-bind:key="picture.imagetype" v-if="picture.imagetype == 'mobile'"
